...
import com.github.alsol.finance.transaction.{TransactionService, TransactionServiceSupport}
...
class Services(using session: Session[IO], logger: LogIO[IO]) {
  ...
  given transactionService: TransactionService = new TransactionServiceSupport()
